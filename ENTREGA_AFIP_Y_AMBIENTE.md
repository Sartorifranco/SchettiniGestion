# ARCA: ambiente (prueba vs producción), `AfipService` y checklist de entrega
...
- En la aplicación WPF: **Configuración** → pestaña donde está **«Negocio»** → sección **«Facturación Electrónica (ARCA)»**.
- Ahí existe el checkbox: **«Ambiente ARCA: producción (no homologación)»** (`chkAfipProduccion`).
  - **Desmarcado** (valor por defecto en base de datos **0/false**): se usa **homologación** (URLs de prueba WSAA / WSFE / padrón).
  - **Marcado**: se usa **producción** (servicios reales ARCA).
...
## Neto e IVA del comprobante ARCA

Para armar los importes que pide ARCA (`ImpNeto`, `ImpIVA`, totales coherentes con el XML), el servicio **no** asume un IVA fijo global del 21 % sobre el total.
...
**Nota:** el bloque XML de ARCA con **`<Iva><AlicIva>…`** puede seguir usando un esquema simplificado (p. ej. un código de alícuota unificado en el nodo). Si en el futuro se requieren **varias alícuotas distintas en un mismo comprobante** con múltiples nodos `<AlicIva>`, habría que extender el armado del XML; los totales agregados ya se calculan por línea en el servicio.

---

## Checklist rápido antes de entregar / ir a producción ARCA
...
2. **Ambiente**
   ...
   - Solo cuando los comprobantes de prueba sean aceptados, marcar **«Ambiente ARCA: producción»** y volver a probar con montos bajos en un entorno controlado.

3. **Punto de venta**
   - **Punto de venta** en configuración alineado con lo dado de alta en ARCA para ese CUIT.
...
*Documento generado para la entrega del producto Schettini Gestión POS; puede actualizarse si cambian pantallas de configuración o reglas ARCA.*
