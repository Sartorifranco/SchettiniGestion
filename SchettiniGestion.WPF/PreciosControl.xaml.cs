...
using System.Windows.Input; // WPF Input (KeyEventArgs)
using Xceed.Wpf.Toolkit;

// Alias para evitar conflicto con WinForms
...
namespace SchettiniGestion.WPF
{
	public partial class PreciosControl : UserControl
	{
		...
		private Control _activeNumericControl = null; // WPF Control
		...
		// --- TECLADO NUMÉRICO ---
		private void NumericInput_GotFocus(object sender, RoutedEventArgs e) { _activeNumericControl = sender as Control; }
		// Eliminamos el LostFocus problemático
		...
		private void numericKeyboard_KeyPressed(object sender, KeyEventArgs e)
		{
			if (_activeNumericControl != null)
			{
				...
				// Envío de teclas usando el ALIAS de WinForms
				string key = e.Key.ToString().Replace("D", "").Replace("NumPad", "");

				if (e.Key == Key.Back) WinForms.SendKeys.SendWait("{BACKSPACE}");
				else if (e.Key == Key.Enter) WinForms.SendKeys.SendWait("{ENTER}");
				else if (e.Key == Key.Decimal || e.Key == Key.OemPeriod) WinForms.SendKeys.SendWait(".");
