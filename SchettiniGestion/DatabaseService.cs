...
namespace SchettiniGestion
{
    ...
    public static class DatabaseService
    {
                ...
                using (var conn = new SQLiteConnection($"Data Source={_dbPath};Version=3;"))
                {
                    ...
                    using (var trans = conn.BeginTransaction())
                    {
                        try
                        {
                            ...
                            // ===== INICIO CÓDIGO NUEVO (LISTAS PRECIOS) =====
                            new SQLiteCommand(@"CREATE TABLE IF NOT EXISTS ListasPrecios (
                                ListaID INTEGER PRIMARY KEY AUTOINCREMENT, 
                                Nombre TEXT NOT NULL UNIQUE, 
                                Porcentaje REAL DEFAULT 0 -- Porcentaje de aumento o descuento sobre el precio base
                            );", conn, trans).ExecuteNonQuery();

                            // Insertar lista base por defecto
                            ...
                            // Asignar permiso de listas a vendedor/cajero si se desea (por ahora solo admin)
                            // new SQLiteCommand($"INSERT OR IGNORE INTO Roles_Permisos (RolID, PermisoID) SELECT 2, PermisoID FROM Permisos WHERE NombrePermiso = '{PERMISO_LISTASPRECIOS}';", conn, trans).ExecuteNonQuery();
        ...
        // --- MÉTODOS NUEVOS: LISTAS DE PRECIOS ---
