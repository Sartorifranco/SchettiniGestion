...
namespace SchettiniGestion
{
    ...
    public static class DatabaseService
    {
                ...
                using (var conn = new SQLiteConnection($"Data Source={_dbPath};Version=3;"))
                {
                    ...
                    using (var trans = conn.BeginTransaction())
                    {
                        try
                        {
                            ...
                            // 2. Tabla Productos (ACTUALIZADA CON NUEVOS CAMPOS)
                            ...
                            // 5. Tablas Operativas (Facturas, Compras, Stock, Caja)
        ...
        // ¡MÉTODO ACTUALIZADO! Recibe todos los datos de la ficha completa (12 argumentos)
        public static bool GuardarProducto(int id, string cod, string codBarra, string desc, string cat, string iva, decimal costo, decimal ganancia, decimal impInt, decimal venta, int stock, string imgPath)
        {
            try
            {
                using (var c = new SQLiteConnection($"Data Source={_dbPath};Version=3;"))
                {
                    c.Open();
                    string sql = id == 0
                        ? "INSERT INTO Productos (Codigo, CodigoBarra, Descripcion, Categoria, TipoIVA, PrecioCosto, Ganancia, ImpuestoInterno, PrecioVenta, StockActual, ImagenPath) VALUES (@c, @cb, @d, @cat, @iva, @pc, @g, @ii, @pv, @s, @img)"
                        : "UPDATE Productos SET Codigo=@c, CodigoBarra=@cb, Descripcion=@d, Categoria=@cat, TipoIVA=@iva, PrecioCosto=@pc, Ganancia=@g, ImpuestoInterno=@ii, PrecioVenta=@pv, StockActual=@s, ImagenPath=@img WHERE ProductoID=@id";

                    using (var cmd = new SQLiteCommand(sql, c))
                    {
                        cmd.Parameters.AddWithValue("@c", cod);
                        cmd.Parameters.AddWithValue("@cb", codBarra);
                        cmd.Parameters.AddWithValue("@d", desc);
                        cmd.Parameters.AddWithValue("@cat", cat);
                        cmd.Parameters.AddWithValue("@iva", iva);
                        cmd.Parameters.AddWithValue("@pc", (double)costo);
                        cmd.Parameters.AddWithValue("@g", (double)ganancia);
                        cmd.Parameters.AddWithValue("@ii", (double)impInt);
                        cmd.Parameters.AddWithValue("@pv", (double)venta);
        ...
        // Actualizado para buscar también por código de barras
        ...
        // Búsqueda múltiple actualizada (incluye código de barras)
        ...
        public static DataTable GetFacturasPorFecha(DateTime d, DateTime h) { var dt = new DataTable(); try { using (var c = new SQLiteConnection($"Data Source={_dbPath};Version=3;")) { c.Open(); new SQLiteDataAdapter($"SELECT f.FacturaID, f.Fecha, c.RazonSocial, f.TipoComprobante, f.Total FROM Facturas f JOIN Clientes c ON f.ClienteID = c.ClienteID WHERE f.Fecha BETWEEN '{d:yyyy-MM-dd} 00:00:00' AND '{h:yyyy-MM-dd} 23:59:59' ORDER BY f.Fecha DESC", c).Fill(dt); } } catch { } return dt; }
        ...
        public static DataTable GetPresupuestos(DateTime d, DateTime h) { var dt = new DataTable(); try { using (var c = new SQLiteConnection($"Data Source={_dbPath};Version=3;")) { c.Open(); new SQLiteDataAdapter($"SELECT p.PresupuestoID, p.Fecha, c.RazonSocial, p.Total, p.Estado FROM Presupuestos p JOIN Clientes c ON p.ClienteID = c.ClienteID WHERE p.Fecha BETWEEN '{d:yyyy-MM-dd} 00:00:00' AND '{h:yyyy-MM-dd} 23:59:59' ORDER BY p.Fecha DESC", c).Fill(dt); } } catch { } return dt; }
